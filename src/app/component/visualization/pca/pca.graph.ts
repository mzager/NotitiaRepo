...
export class PcaGraph extends AbstractVisualization {

    // Chart Elements
    private data: PcaDataModel;
    private config: PcaConfigModel;
    ...
    update(config: GraphConfig, data: any) {
        this.config = config as PcaConfigModel;
        this.data = data;
        this.removeObjects();
        this.addObjects();
    }
    ...
    addObjects() {
        const weightLength = this.data.scoresScaled.length;
        const layoutLength = this.data.scoresScaled.length;
        ...
        if (this.config.showVectors) {
            ...
            for (let i = 0; i < weightLength; i++) {
                const position = this.data.scoresScaled[i];
