...
} from './parallelcoords.model';

export class ParallelCoordsGraph extends AbstractVisualization
    ...
    if (this.config.dirtyFlag & DirtyEnum.COLOR) {
      ...
      this.meshes.forEach(v => {
        if (data.pointColor.hasOwnProperty(v.userData.gene)) {
          ...
          (v as THREE.Mesh).material = new THREE.MeshBasicMaterial({
            color: color
          });
        } else {
          (v as THREE.Mesh).material = new THREE.MeshBasicMaterial({
      ...
      lines.forEach(v => {
        if (data.pointColor.hasOwnProperty(v.userData.gene)) {
          const color = data.pointColor[v.userData.gene];
          (v as THREE.Line).material = new THREE.LineBasicMaterial({
            color: color
          });
        } else {
          (v as THREE.Line).material = new THREE.LineBasicMaterial({
