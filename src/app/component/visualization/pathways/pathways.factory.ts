...
export class PathwaysFactory {
  public static createEdge(edge: any): Object3D {
    ...
    const offset = new Vector2(Math.cos(radians) * dim.x * 0.5, Math.sin(radians) * dim.y * 0.5);
    switch (edge.class) {
  ...
  private static createRoundedRectangleShape(
    w: number,
    h: number,
    x: number,
    y: number
  ...
  private static createConsumption(x1, y1, x2, y2, xO, yO): THREE.Group {
    ...
    const squareGeom = new THREE.ShapeGeometry(
      new THREE.Shape([new Vector2(0, 0), new Vector2(0, 5), new Vector2(5, 5), new Vector2(5, 0)])
    );
    const square = new THREE.Mesh(squareGeom, ChartFactory.getColorPhong(color));
  ...
  private static createCatalysis(x1, y1, x2, y2, xO, yO): THREE.Group {
    ...
    group.add(line);
    const rotation = Math.atan2(y2 - y1, x2 - x1);
    const circle = new THREE.Mesh(new THREE.CircleGeometry(2.5), ChartFactory.getColorPhong(color));
  ...
  private static createProduction(x1, y1, x2, y2, xO, yO): THREE.Group {
    ...
    const triangleGeom = new THREE.ShapeGeometry(
      new THREE.Shape([new Vector2(0, 0), new Vector2(6, -3), new Vector2(6, 3)])
    );
    const triangle = new THREE.Mesh(triangleGeom, ChartFactory.getColorPhong(color));
  ...
  private static createStimulation(x1, y1, x2, y2, xO, yO): THREE.Group {
    ...
    const triangle = new THREE.Mesh(triangleGeom, ChartFactory.getColorPhong(0xffcc80));
