...
export class ComputeWorkerUtil {
  ...
  getCytobands(alignment: string): Promise<any> {
    return fetch(
      'https://oncoscape.v3.sttrcancer.org/data/reference/hg-' + alignment + '-cytoband.json.gz',
      {
        method: 'GET',
  ...
  getGenes(alignment: string): Promise<any> {
    return fetch(
      'https://oncoscape.v3.sttrcancer.org/data/reference/hg-' + alignment + '-genes.json.gz',
      {
        method: 'GET',
        headers: this.headersJson
      }
      ...
      this.openDatabaseData(db).then(v => {
        ...
        if (entity === EntityTypeEnum.GENE) {
          if (field.ctype & CollectionTypeEnum.MOLECULAR) {
            ...
            fetch(
              'https://dev.oncoscape.sttrcancer.io/api/z_gene_families/' + JSON.stringify(query),
              {
                method: 'GET',
                headers: {
                  Accept: 'application/json',
                ...
                this.dbData
                  ...
                  .then(sampleMap => {
                    this.getMolecularGeneValues(markers, field, db).then(result => {
                      ...
                      legend.type = 'COLOR';
                      legend.display = 'CONTINUOUS';
                      legend.labels = [sampleDomain.min, sampleDomain.max].map(val =>
                        Math.round(val).toString()
                      );
                      legend.values = [0xff0000, 0xff0000];
