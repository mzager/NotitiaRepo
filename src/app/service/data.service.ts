...
import { Injectable } from "@angular/core";
import { QueryBuilderConfig } from "app/component/workspace/query-panel/query-builder/query-builder.interfaces";
import { CollectionTypeEnum, EntityTypeEnum, SpriteMaterialEnum, VisualizationEnum, DataTypeEnum } from "app/model/enum.model";
import { GraphConfig } from "app/model/graph-config.model";
import * as d3 from "d3";
import Dexie from "dexie";
import * as JStat from "jstat";
...
<<<<<<< HEAD
          Promise.all([db.table(tbl).toArray(), db.table('patientSampleMap').toArray()]).then(
            results => {
            ...
            db.table('genecoords')
              ...
              .then(results => {
                ...
                decorator.legend.type =
                  decorator.type === DataDecoratorTypeEnum.SHAPE ? 'SHAPE' : 'COLOR';
          ...
          new Dexie("notitia").open().then(db => {
            db.table("genecoords")
              ...
              .then(results => {
                ...
                const scale = decorator.type === DataDecoratorTypeEnum.SHAPE ? ChartFactory.getScaleShapeOrdinal(DataService.biotypeCat) : ChartFactory.getScaleColorOrdinal(DataService.biotypeCat);
                decorator.values = results
                  .map(v => ({
                    pid: null,
                    sid: null,
...
=======
              decorator.legend.type = "COLOR";
              decorator.legend.display = "DISCRETE";
              decorator.legend.name = config.entity === EntityTypeEnum.SAMPLE ? "Sample " + decorator.field.label : config.entity === EntityTypeEnum.GENE ? "Gene " + decorator.field.label : "Patient " + decorator.field.label;
              if (decorator.field.type === "STRING") {
                decorator.legend.labels = scale["domain"]().filter(v => v);
                if (!decorator.legend.labels.find(v => v === "NA")) {
                  decorator.legend.labels.concat(["NA"]);
...
<<<<<<< HEAD
          ...
          .sort(
            (a, b) =>
              a.type !== b.type ? a.type.localeCompare(b.type) : a.name.localeCompare(b.name)
          );
        ...
        db.open().then(connection => {
          ...
          let text =
            '<p>You selected ' + pids.length + ' patients ' + ' from ' + sids.length + ' samples ';
...
=======
  ...
  getPatientIdsWithQueryBuilderCriteria(database: string, config: QueryBuilderConfig, criteria: { condition: string; rules: Array<{ field: string; operator: string; value: any }> }): Promise<Array<string>> {
    return new Promise(resolve => {
      const db = new Dexie("notitia-" + database);
      db.open().then(connection => {
        Promise.all(
...
=======
              const allGenes = Array.from(new Set([...Array.from(new Set(geneLinksData.map(gene => gene.source))), ...Array.from(new Set(geneLinksData.map(gene => gene.target)))]));
>>>>>>> a947a94743ca0203655e535e63bac428d1227be3
...
<<<<<<< HEAD
                ...
                link.source = validHugoGenes.has(link.source)
                  ...
                  : missingMap.hasOwnProperty(link.source)
                    ...
                    : null;
=======
                link.target = validHugoGenes.has(link.target) ? link.target : missingMap.hasOwnProperty(link.target) ? missingMap[link.target].hugo : null;
                link.source = validHugoGenes.has(link.source) ? link.source : missingMap.hasOwnProperty(link.source) ? missingMap[link.source].hugo : null;
>>>>>>> a947a94743ca0203655e535e63bac428d1227be3
              });
