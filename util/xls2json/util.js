...
exports.loadCsv = file => {
  let data = fs
    .readFileSync(process.cwd() + '/data/' + file, 'UTF-8')
    .split('\n')
    .map(v => v.split('","'));
  data.forEach(v => {
    v[0] = v[0].substring(1);
...
exports.extractColumnValues = (data, columnIndex, formatter) => {
  return Array.from(new Set(data.map(v => formatter(v[columnIndex]))))
    .filter(v => v !== '')
    .filter(v => v !== null);
};
...
exports.formatMut = value => {
  ...
  value = value
    ...
    .trim()
    .replace(/\s/gi, '_');
  if (!mutationType.hasOwnProperty(value)) {
    return 33554432;
  }
