...
export class Validate {
  public static Run(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      this.processPatient('./src/output/patient.csv').then(
        (patientIds: Array<string>) => {
          this.processSample('./src/output/sample.csv', patientIds).then(
  ...
  private static ExtractPropertyValues(
  ...
  public static Mutation(
    ...
    return new Promise((resolve, reject) => {
      ...
      // .map<iTest>(Test.uniqueProperties(['symbol']));
      ...
      // Load Meta Data
      IO.loadMetadata(results, ['sample id', 'hgnc', 'variant']).then(
        metadata => {
          ...
          setTimeout(v => {
            ...
            Promise.all([
              IO.WriteLog(baseFileName + '.meta.log.json', results.observe()),
              IO.WriteJson(baseFileName + '.meta.raw.json', results)
            ]).then(() => resolve());
          }, 30000);
        }
      );
  ...
  public static Event(
    ...
    return new Promise((resolve, reject) => {
      ...
      // Load Meta Data
      IO.loadMetadata(results.fork(), ['patient id', 'start', 'end']).then(
        metadata => {
          const results = metadata
            .map<iTest>(Test.metaFieldsWithOneValue())
            .map<iTest>(Test.metaFieldLabels())
  ...
  public static processSample(
    ...
    return new Promise((resolve, reject) => {
      ...
      // Load Meta Data
      IO.loadMetadata(results.fork(), ['patient id', 'sample id']).then(
        metadata => {
          const results = metadata
            .map<iTest>(Test.metaFieldsWithOneValue())
            .map<iTest>(Test.metaFieldLabels())
  ...
  public static processPatient(uri: string): Promise<Array<string>> {
    return new Promise((resolve, reject) => {
      ...
      IO.loadMetadata(results.fork(), ['patient id']).then(metadata => {
        ...
        // Write Meta Data
        IO.WriteJson('patient.meta.raw.json', results.observe());
        IO.WriteLog('patient.meta.log.json', results);
...
export class Test {
   ...
   */
  public static propertyValuesInSet(
    prop: string,
    values: Array<string>
  ): (x: iTest) => iTest {
    ...
    return (obj: iTest) => {
      ...
      if (stringValues > 0 && numberValues > 0) {
        ...
        if (totalValues < 12) {
          ...
          obj.data.type =
            stringValues === 0 ? eDataType.Number : eDataType.String;
        } else {
          ...
          if (stringValues <= 3) {
            ...
            obj.error.push({
              ...
              value:
                ...
                'values, of which ' +
                Math.round(100 * percentNumeric).toString() +
                '% are numeric'
            });
