...
var WriteJson = /** @class */ (function () {
        ...
        return new Promise(function (resolve, reject) {
            ...
            Promise.all(files.map(function (v) {
                return new Promise(function (resolve, reject) {
                    ...
                    var samplesInMatrix = samples.filter(function (v) {
                        return matrix[0].data.hasOwnProperty(v);
                    });
                    var dataGenes = matrix.reduce(function (p, c) {
                        var datum = c.data;
                        p.genes.push(datum.symbol);
                        p.data.push(samplesInMatrix.map(function (sample) {
                            return parseFloat(datum[sample]);
                        }));
                        return p;
                    }, { data: [], genes: [] });
    ...
    WriteJson.writeSampleJson = function (uri) {
        return new Promise(function (resolve, reject) {
            var meta = IO_1.IO.ReadJson('./src/output/', 'sample.meta.raw.json');
            var fields = meta
    ...
    WriteJson.writePatientJson = function (uri) {
        return new Promise(function (resolve, reject) {
            var meta = IO_1.IO.ReadJson('./src/output/', 'patient.meta.raw.json');
            var fields = meta
                .filter(function (v) { return v.error.length === 0; })
                .reduce(function (p, c) {
                if (c.data.type === InterfacesAndEnums_1.eDataType.Number) {
